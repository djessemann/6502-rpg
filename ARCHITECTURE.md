...
| File                  | Role                                                          |
|-----------------------|---------------------------------------------------------------|
| `src/header.s`        | iNES header (mapper 0, 32KB PRG, 8KB CHR, vertical mirroring). |
| `src/nes.inc`         | Hardware register constants.                                  |
| `src/main.s`          | Everything: boot, main loop, NMI, state machine, routines, palettes, tables. |
| `src/chr.s`           | **Generated.** CHR-ROM tile data (BG table 0; hero + cat table 1). |
...
| `src/tiles.inc`       | **Generated.** Tile/geometry constants and message ids (`HERO_*_TILE`, `CAT_TILE_BASE`, `ENEMY_TILE_BASE`, `WIN_*_TILE`, `MSG_*`). |
...
bush, water — each 4 subtiles, authored as true 16×16 art in `TILES16{}`); the
**NPC** metatile (4 subtiles, its authored facing); the window frame (9 tiles);
the font glyphs; the enemy (4×4, authored 32×32). `main.s` hardcodes no terrain
ids — the nametables reference them directly and collision uses `worldsolid` —
so only `ENEMY_TILE_BASE`, the `WIN_*_TILE` frame tiles, `ARROW_TILE`,
`DIGIT_TILE`, `CURSOR_TILE` are emitted to `tiles.inc` for the text/battle code.

Pattern table 1 (sprites, `$1000`): the hero, 3 facings × 2 walk frames × 2x2 —
frame 0 down `$00`, up `$04`, side `$08` (left = side flipped at draw time),
frame 1 at `+HERO_FRAME_OFF` (`$0C`); then the cat, 4 frames × 2x2 from
`CAT_TILE_BASE` (`$18`), frame f at `+f*4`. `PPUCTRL` selects BG table 0 /
sprite table 1 (`%10001000`).

These ranges are emitted to **`src/tiles.inc`** by `gen_assets.py`; `main.s`
includes it, so no tile numbers are hardcoded in the assembly.
...
- **Sprites** (hero, moving NPCs): `BuildOAM` is table-driven by `ent_dir`
  (`dir_tiles`, `dir_attr`, `slot_dx/dy`). Left reuses the side tiles with the
  OAM **H-flip bit** ($40) set and the two tile **columns swapped**. The hero
  has **two walk frames per view** and animates constantly (DQ-style): frame 1
  = tile `+HERO_FRAME_OFF`, toggled from `frame_count` every 8 frames while
  sliding (one stride per 16px step), every 16 at rest.
...
- **One-view sprites** (the cat): a single right-facing view × 4 animation
  frames; `BuildCatOAM` cycles frames from `frame_count` (~7.5 fps) and
  H-flips the whole metasprite when it last walked left (up/down reuse the
  current horizontal facing).
...
```
ReadInput → VBufClear → UpdateGame → (BuildOAM + BuildCatOAM | HideHero) → WaitFrame → loop
```
...
```
GS_FIELD(0)      walk (hero + CatTick + WaterTick); A → command menu; Select/steps → EnterBattle
...
GS_MENU(10)      command/equip/battle menu shown; cursor input (up/down/A/B)
```

`msg_context` (FIELD/BATTLE/MENU) decides where `GS_TEXT` goes when a message
ends: FIELD→`GS_DIALOG`, MENU→`GS_MENU`, BATTLE→`GS_BWAIT` (except the battle
intro, which opens the Fight/Run menu directly). The hero and cat sprites are
hidden whenever `in_battle` is set, drawn otherwise.
...
(Talk→dialogue if `FacingNPC` else `MSG_NOBODY`; Equip→`MENU_EQUIP`),
`MENU_EQUIP` (pick a weapon → sets `equipped`/`player_atk` from `weapon_atk[]`,
shows a live "Power"), and `MENU_BATTLE` (Fight/Run; B can't back out). This
one menu/cursor primitive serves field commands and battle alike.

### Battle flow

`EnterBattle` cuts to the black arena: `DrawBattle` draws the 4×4-tile enemy
and the **status window** (rows 2-4: "HP dd" via `ComposeHpRow`), sets
`enemy_hp`, opens the box and shows `MSG_SLIME_APPEARS`; when the intro ends,
`OpenBattleMenu` shows the Fight/Run menu. The loop, driven by `battle_phase`
in `GS_BWAIT`:

- **Fight** → `DoAttack` (`player_atk` with a −1 frame-parity wobble, capped to
  remaining HP; `ComposeDamage` builds "The Slime takes N damage!") → A → the
  slime counterattacks for 1-2 (`ComposeEnemyHit`, `player_hp` floors at 0; the
  status window redraws via a VRAM packet flushed in `GS_BWAIT` when `hp_dirty`
  is set) → A → menu again.
- Enemy at 0 HP → `MSG_SLIME_DEFEATED` → erase the enemy → `ExitBattle`.
- **Run** → `MSG_FLEE` → `ExitBattle`.
- Hero at 0 HP → `MSG_DEAD` → revive at `PLAYER_MAX_HP` → `ExitBattle`.

`player_hp` persists between battles; `ExitBattle` returns to the field at the
prior spot.
...
tiles (TL,TR,BL,BR), split from **true 16×16 authored art** (`TILES16{}` in the
...
- `camY = heroWorldY − 112 (mod 480)` → `scrollY = camY mod 240` (PPUSCROLL Y) +
  `cam_ty = camY/8` (the camera's top tile-row, for the streamer).
...
4 BG + 4 sprite sub-palettes each, black ($0F) backdrop on both screens.
Field BG: 0 ground (greens + brown), 1 water (`WaterTick` palette-cycles its
two blues every 16 frames for a shimmer), 2 deco (NPC + flowers: green, gold,
red), 3 window (white ink, red/tan — brick walls share it). Field sprites:
0 hero, 1 cat. Battle BG: 1 = enemy, 3 = window. **Every sub-palette's color-0
entry must equal the backdrop** because of the $3F1x→$3F0x mirror (see
CLAUDE.md trap).
...
Hand-authored art (digit grids: 16×16 for terrain/characters, 32×32 for the
enemy, `'#'/'.'` for glyphs) → encodes 2bpp planar CHR + the world tilemaps,
...
External sprite exports (e.g. nesprite CHR) are decoded into the same digit-grid
format so everything stays editable in one place (see `CAT_FRAMES`).
...
|------|--------------------------|
| add / change a terrain tile  | `TILES16{}` (true 16×16 digit art, one metatile) + `cell_pal` |
| recolor                      | the palette tables (`pal_field` / `pal_battle` in `main.s`) + `cell_pal` |
| change the map               | the `world[][]` build (terrain + `SOLID` set) |
| place / face the BG NPC      | `NPC_GX`, `NPC_GY`, `NPC_FACING` |
| add a character facing set   | `HERO_VIEWS` (down/up/side × 2 walk frames) / `NPC_VIEWS` (left is mirrored) |
| change the wandering cat     | `CAT_FRAMES` (4 one-view frames); home box / speed in `main.s` (`CAT_*`, `CatTick`) |
| add / change a monster       | `ENEMY32` (true 32×32) |
...
- **Entity / NPC / encounter formats.** Replace the hardcoded NPC, cat and enemy.
- **Stats + a real battle system**, replacing the slice battle loop
  (`battle_phase` in `GS_BWAIT`/`ENEMYDIE`/`BATTLEWAIT`).
