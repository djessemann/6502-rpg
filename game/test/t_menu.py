...
def build(rom, extra=()):
    defs = ["-D", f"TEST_START_DUNGEON={world.MAP_ID['LANDFALL']}",
            ...
            "-D", "TEST_NO_ENCOUNTERS=1", "-D", "TEST_LIST_N=3"] + list(extra)
...
r.tap(DOWN, 3, 20)
r.tap(DOWN, 3, 20)
r.tap(A, 3, 40)                          # TECH -> whose
r.tap(DOWN, 3, 20)
r.tap(DOWN, 3, 20)                       # the MEDIC
r.tap(A, 3, 50)
techs = r.frame
r.shot("menu_6_tech")
...
r.tap(DOWN, 3, 20)
r.tap(A, 3, 40)                          # CLEANSE: no heal, no cure mask
check(glyphs.says(r.frame, "IN BATTLE ONLY"),
      "a tech with nothing to do out of battle says so")
r.tap(UP, 3, 20)
r.tap(A, 3, 40)                          # MEND -> on whom
check(glyphs.line(r.frame, 1).strip() == "ON WHOM", "MEND asks for a target")
r.tap(A, 3, 50)
check(glyphs.says(r.frame, "DONE"), "and casts")
r.tap(B, 3, 40)
r.tap(B, 3, 40)
roster = r.frame
check(glyphs.line(roster, 10).strip().endswith("TP   7/  8"),
      f"the MEDIC's TP went 8 -> 7 ({glyphs.line(roster, 10).strip()!r})")
