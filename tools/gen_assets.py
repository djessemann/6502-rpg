...
TILES16 = {
    # palette 0: 0=black outline/tufts, 1=dark grass green, 2=light foliage
    # green (tree/bush canopies), 3=brown
    T_GRASS: [
        "1111111111111111",
        "1111111111111111",
        "1110011111111111",
...
# ---------------------------------------------------------------------------
# Cat: a wandering sprite NPC. Imported from a nesprite export (16x16, 2x2
# tiles, 4 frames, 8 fps); decoded from its CHR bytes into the pipeline's art
# format. One authored facing (RIGHT); the runtime H-flips it to walk left.
# Value 0 = transparent. Sprite palette 1 (from the export):
# 1=pink ($24), 2=red ($15), 3=black ($0F).
# ---------------------------------------------------------------------------
CAT_FRAMES = [
    [   # frame 0
        "0000000001000010",
...
def main():
    ...
    # Cat sprite frames follow the hero: 4 frames x 4 tiles from CAT_TILE_BASE.
    cat_base = len(hero_bytes) // 16
    for frame in CAT_FRAMES:
        assert len(frame) == 16 and all(len(r) == 16 for r in frame)
        for tile in split16(frame):
            hero_bytes += to_chr(tile)
    ...
    with open(os.path.join(src, "tiles.inc"), "w") as f:
        ...
        f.write(f"CAT_TILE_BASE  = ${cat_base:02X}   ; 4 frames x 4 tiles, frame f at +f*4\n\n")
